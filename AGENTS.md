...
Текущая версия: v60 (сентябрь 2026). Тестов: 106 (`logic` + `ios` + `sync` + `sync-devices` + `journal`).
...
- Ручной порядок (v56, фикс v57): `moveCategory`/`moveProduct` переносят со сдвигом,
  ...
  пунктом ⚙ → «Порядок», выход — повторный тап, тап по шапке или смена Д/С.
- Проверка новой версии (v60, `checkUpdate`): качает `app.js?nocache=` мимо кэша SW
  (в sw.js явный bypass), при открытии и возврате на вкладку, не чаще раза в 5 минут.
  Находит новее — спрашивает «Вышла новая версия — обновить?» и чистит кэш сам.

## UI-договорённости (не ломать молча)

- Кнопка Д/С в шапке переключает режимы; отдельных кнопок режимов нет, drawer'а нет.
- Настройки (repo/token) живут в ⚙; там же диагностика (кнопка «Поделиться»),
  ручной синк, установка PWA, поделиться списком (`shareText`: `* категория:`,
  `товар - qty`, пустая строка между группами), порядок (`sortMode`,
  стрелки ⏮ ⏭ ‹ ›), 3 очистки.
