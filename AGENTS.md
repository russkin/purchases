...
Текущая версия: v56 (сентябрь 2026). Тестов: 102 (`logic` + `ios` + `sync` + `sync-devices` + `journal`).
...
- Ручной порядок (v56): `moveCategory`/`moveProduct` переносят со сдвигом, свежие `ts`
  ставятся всему затронутому диапазону (слияние позиционное). Режим включается пунктом
  ⚙ → «Порядок», выход — повторный тап или смена Д/С.
