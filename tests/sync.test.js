...
describe('свой PHP-сервер вместо GitHub', () => {
  const SRV = 'https://example.test/server/sync.php';
  function srvState(catalog, t, rev) {
    return { catalog, settings: { mode: 'add', repo: 'r/x', token: 'TOK', server: SRV }, updatedAt: t };
  }
  function srvResp(rev, state) {
    return { status: 200, ok: true, json: async () => ({ rev, state }) };
  }

  it('пустой сервер + локальные данные — pushed на свой URL', async () => {
    const fetch = stubFetch(async (url, opts) => {
      assert.ok(url.startsWith(SRV), 'запрос не на свой сервер: ' + url);
      if (opts.method === 'PUT') return { status: 200, ok: true, json: async () => ({ rev: 'R1' }) };
      return { status: 404, ok: false, json: async () => ({}) };
    });
    const api = factory(L, fetch);
    const res = await api.syncNow(srvState(L.seedCatalog(), 100));
    assert.equal(res.status, 'pushed');
    const put = fetch.calls.find((c) => c.opts.method === 'PUT');
    const body = JSON.parse(put.opts.body);
    assert.ok(body.state && body.state.catalog, 'нет state в теле');
  });

  it('удалённые данные забираются — pulled', async () => {
    const remote = seedAt(200);
    L.incProduct(remote.catalog, 0, 0, 200);
    const fetch = stubFetch(async () => srvResp('R9', remote));
    const api = factory(L, fetch);
    const local = srvState(L.seedCatalog(), 100);
    const res = await api.syncNow(local);
    assert.equal(res.status, 'pulled');
    assert.equal(res.state.catalog.categories[0].products[0].qty, 1);
  });

  it('409 своего сервера — ретрай и pushed', async () => {
    const remote = seedAt(100);
    let puts = 0;
    const fetch = stubFetch(async (url, opts) => {
      if (opts.method === 'PUT') {
        puts += 1;
        if (puts === 1) return errResp(409, 'conflict');
        return { status: 200, ok: true, json: async () => ({ rev: 'R2' }) };
      }
      return srvResp('R1', remote);
    });
    const api = factory(L, fetch, { baseMs: 1, capMs: 5, jitterMs: 0 });
    const local = srvState(L.seedCatalog(), 200);
    L.incProduct(local.catalog, 0, 0, 200);
    const res = await api.syncNow(local);
    assert.equal(res.status, 'pushed');
    assert.equal(puts, 2);
  ...
  it('без server в настройках — ходим на GitHub как раньше', async () => {
    const fetch = stubFetch(async () => ({ status: 404, ok: false, json: async () => ({}) }));
    const api = factory(L, fetch);
    await api.syncNow(stateWith(L.blankCatalog(), 100));
    assert.ok(fetch.calls[0].url.includes('api.github.com'), 'не GitHub URL');
  });
});
